...
impl KuwoProvider {
    ...
    pub async fn resolve_url(&self, track: &MusicTrack, quality: &str) -> Result<String, String> {
        ...
        value
            ...
            .map(str::to_owned)
            .ok_or_else(|| "平台没有返回可播放地址，请导入自定义源".into())
    }
...
fn artwork_url(value: &Value) -> Option<String> {
    ...
    Some(format!(
        "https://img1.kuwo.cn/star/albumcover/500/{normalized}"
    ))
}
...
fn parse_track(value: Value) -> Option<MusicTrack> {
    ...
    for (needle, quality) in [
        ("bitrate:4000", "flac24bit"),
        ("bitrate:2000", "flac"),
        ("bitrate:320", "320k"),
        ("bitrate:128", "128k"),
    ] {
        if quality_info.contains(needle) {
            qualities.push(quality.to_owned());
        }
    }
    if qualities.is_empty() {
        qualities.push("128k".into());
    }
