...
<template>
  <footer class="player-bar">
    <div class="now-playing"><ArtworkImage :src="player.state.current?.artworkUrl" :alt="player.state.current ? `${player.state.current.name} 封面` : ''" size="player"/><div class="track-copy"><strong>{{ player.state.current?.name || '暂未播放' }}</strong><span>{{ player.state.current?.artist || '选择一首歌曲开始聆听' }}</span></div></div>
