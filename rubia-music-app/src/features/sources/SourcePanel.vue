...
<template>
  <div v-if="sourceSettingsOpen" class="modal-backdrop" @click.self="sourceSettingsOpen=false">
    <section class="settings-modal">
      <aside class="settings-nav"><h2>设置</h2><button :class="{ active: activeSection === 'general' }" @click="activeSection='general'"><AppIcon name="settings"/>通用</button><button :class="{ active: activeSection === 'playback' }" @click="activeSection='playback'"><AppIcon name="play"/>播放</button><button :class="{ active: activeSection === 'lyrics' }" @click="activeSection='lyrics'"><AppIcon name="lyrics"/>歌词</button><button :class="{ active: activeSection === 'source' }" @click="activeSection='source'"><AppIcon name="source"/>音乐来源</button><button :class="{ active: activeSection === 'data' }" @click="activeSection='data'"><AppIcon name="database"/>数据</button></aside>
      <main class="settings-content">
        <button class="modal-close" aria-label="关闭设置" @click="sourceSettingsOpen=false"><AppIcon name="close" :size="15"/></button>
        ...
        <section v-else-if="activeSection === 'data'" class="settings-page"><p class="eyebrow">数据</p><h1>数据管理</h1><p class="settings-intro">管理保存在这台设备上的资料库和偏好。</p><div class="preference-group"><label class="setting-row"><span><strong>资料库数据</strong><small>收藏 {{ library.favorites.value.length }} 首 · {{ library.playlists.value.length }} 个歌单 / {{ library.playlistTrackCount.value }} 首 · 最近播放 {{ library.recent.value.length }} 首</small></span><button class="danger-button" @click="clearLibrary">清空</button></label><label class="setting-row"><span><strong>恢复默认设置</strong><small>不会删除音乐源和资料库</small></span><button class="secondary-button" @click="resetAppSettings">恢复</button></label></div></section>
        ...
        <div class="source-list">
          <article v-for="source in runtime.sources.value" :key="source.id" class="source-card managed" :class="{ selected: runtime.activeSourceId.value === source.id }" @click="select(source.id)">
            ...
            <button class="delete-source" title="删除" @click.stop="remove(source.id, source.name)"><AppIcon name="trash" :size="16"/></button>
          </article>
        </div>
