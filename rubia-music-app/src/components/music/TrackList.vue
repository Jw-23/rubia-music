...
<template>
  <div class="track-table">
    ...
    <div v-for="(track,index) in tracks" :key="track.source+track.id" class="track" :class="{ playing: player.state.current?.id === track.id && player.state.current?.source === track.source }" @dblclick="player.play(track, props.tracks)">
      ...
      <span class="track-actions"><button title="播放" @click.stop="player.play(track, props.tracks)"><AppIcon name="play" :size="15"/></button><button :class="{ cached: cache.isCached(track) }" :disabled="cache.downloading.value[cache.keyOf(track)] !== undefined || cache.isCached(track)" :title="cache.isCached(track) ? '已离线保存' : '下载离线歌曲'" @click.stop="download(track)"><AppIcon :name="cache.isCached(track) ? 'check' : 'download'" :size="16"/></button><button :class="{ liked: library.isFavorite(track) }" :title="library.isFavorite(track) ? '取消收藏' : '收藏'" @click.stop="library.toggleFavorite(track)"><AppIcon name="heart" :size="16"/></button><details v-if="!removable" class="playlist-picker" @click.stop><summary title="添加到歌单"><AppIcon name="plus" :size="16"/></summary><div><small>添加到歌单</small><button v-for="playlist in library.playlists.value" :key="playlist.id" :class="{ added: library.isInPlaylist(track, playlist.id) }" @click="addTo(track, playlist.id, $event)"><span>{{ playlist.name }}</span><i><AppIcon v-if="library.isInPlaylist(track, playlist.id)" name="check" :size="13"/><template v-else>{{ playlist.tracks.length }}</template></i></button></div></details><button v-else title="从当前歌单移除" @click.stop="library.removeFromPlaylist(track, props.playlistId)"><AppIcon name="minus" :size="16"/></button></span>
    </div>
  </div>
</template>
